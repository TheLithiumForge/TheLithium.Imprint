...
public sealed class DefaultSnapshotComparer : ISnapshotComparer
{
        ...
        switch (left.ValueKind)
        {
            case JsonValueKind.Object:
                ...
                foreach (var name in a.Keys.Order(StringComparer.Ordinal))
                {
                    ...
                    if (!b.TryGetValue(name, out var other)) return child + ": property is missing.";
                ...
                foreach (var name in b.Keys.Order(StringComparer.Ordinal))
                    if (!a.ContainsKey(name)) return path + ": unexpected property " + Excerpt(name) + ".";
                return null;
    ...
    private static bool Augment(int row, bool[,] edges, int[] assigned, bool[] visited, Budget budget)
    {
        ...
        {
            if (!edges[row, j] || visited[j]) continue;
            visited[j] = true;
    ...
    private static Number ParseNumber(string value)
    {
        ...
        if (e < 0) e = value.IndexOf('E');
        var exponent = e < 0 ? BigInteger.Zero : BigInteger.Parse(value[(e + 1)..], CultureInfo.InvariantCulture);
        if (e >= 0) value = value[..e];
        var dot = value.IndexOf('.');
        if (dot >= 0) { exponent -= value.Length - dot - 1; value = value.Remove(dot, 1); }
        value = value.TrimStart('0');
        if (value.Length == 0) return new("0", BigInteger.Zero, false);
    ...
    private static bool NumbersEqual(string left, string right, decimal tolerance)
    {
        ...
        BigInteger Scale(Number number)
        {
            ...
            if (number.Negative) mantissa = -mantissa;
    ...
    {
        if (options.IgnoreLineEndings) value = value.Replace("\r\n", "\n").Replace('\r', '\n');
    ...
    private static string Excerpt(string value)
    {
        ...
        if (escaped.Length > 160) escaped = escaped[..160] + "...";
