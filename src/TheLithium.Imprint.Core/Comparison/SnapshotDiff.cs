...
internal static class SnapshotDiff
{
    ...
    internal static string Create(string expected, string received)
    {
        ...
        for (var i = start; i < prefix; i++) lines.Add(new(' ', left[i]));

        var abbreviated = (long)(a + 1) * (b + 1) > MaxCells || a + b > 10_000;
        if (abbreviated)
        {
            // Never allocate a quadratic matrix for a large changed region.
            for (var i = 0; i < Math.Min(a, 15); i++) lines.Add(new('-', left[prefix + i]));
            for (var i = 0; i < Math.Min(b, 15); i++) lines.Add(new('+', right[prefix + i]));
        }
        else
        {
            ...
            while (x < a || y < b)
            {
                if (x < a && y < b && left[prefix + x] == right[prefix + y])
                {
                    ...
                    lines.Add(new('-', left[prefix + x++]));
                else lines.Add(new('+', right[prefix + y++]));
            }
        }
        for (var i = 0; i < Math.Min(suffix, Context); i++) lines.Add(new(' ', left[left.Length - suffix + i]));
        ...
        while (at < lines.Count)
        {
            ...
            {
                if (lines[end].Kind != ' ') lastChange = end;
                if (end - lastChange > Context * 2) break;
            }
            if (end < lines.Count) end = lastChange + Context + 1;
            var oldCount = 0;
            var newCount = 0;
            for (var i = first; i < end; i++)
            {
                if (lines[i].Kind != '+') oldCount++;
                if (lines[i].Kind != '-') newCount++;
            }
            ...
            var nextChange = at;
            while (nextChange < lines.Count && lines[nextChange].Kind == ' ') nextChange++;
            var nextStart = Math.Max(at, nextChange - Context);
            oldLine += nextStart - at;
            newLine += nextStart - at;
            at = nextStart;
        }
        if (abbreviated) output.Append("... large diff abbreviated; inspect the full expected and received artifacts.\n");
