...
{
    private enum State { Open, Completing, Completed, Faulted, Aborted }
    ...
    public SnapshotUpdate Update
    {
        ...
        set
        {
            lock (_gate)
            {
                ...
                if (_started) throw new SnapshotConfigurationException("Set the test update policy before the first capture.");
        ...
        string? expression, SnapshotWriter<T>? writer)
    {
        lock (_gate)
        {
            ...
            try
            {
                ...
                if (_values.Count >= 1024) throw new SnapshotCaptureException("A test may capture at most 1024 values.");
                ...
                if (_bytes > Math.Max(_settings.MaxBytes, 128 * 1024 * 1024))
                ...
                {
                    SnapshotFormat.Json => ".json", SnapshotFormat.Text => ".txt", _ => ".snap"
                };
    ...
    private string ResolveName(string? explicitName, string? expression)
    {
        ...
        {
            string sequential;
            do { sequential = "snapshot-" + (++_unnamed).ToString(System.Globalization.CultureInfo.InvariantCulture); }
            ...
            try
            {
                ...
                foreach (var value in _values.OrderBy(x => x.Name, StringComparer.Ordinal))
                {
                    ...
                    if (candidates.Length == 0)
                    {
                        ...
                        entries.Add(entry);
                        approvedEntries.Add(entry with { Status = SnapshotStatus.Created, Difference = null });
                    ...
                    else comparison = (value.Comparer ?? DefaultSnapshotComparer.Instance)
                    ...
                    if (!comparison.Equal && policy != SnapshotUpdate.All) authorized = false;
                    if (policy == SnapshotUpdate.All && (existing != value.FileName || _baseline.Files[existing] != value.Text))
                    {
                        desired.Remove(existing);
                        desired[value.FileName] = value.Text;
                        changes = true;
                        approvedEntries.Add(entryResult with { Status = SnapshotStatus.Updated, Difference = null });
                    }
                    else approvedEntries.Add(entryResult);
                }
                ...
                foreach (var existing in _baseline.Files.Keys.Where(x => !used.Contains(x)).Order(StringComparer.Ordinal))
                {
                    ...
                    entries.Add(entry);
                    approvedEntries.Add(entry with { Status = SnapshotStatus.Removed, Difference = null });
                    if (testPolicy == SnapshotUpdate.All) { desired.Remove(existing); changes = true; }
                    else authorized = false;
                }
                if (!authorized)
                {
                    ...
                    report = report with { ArtifactDirectory = ArtifactDirectory };
                    throw new SnapshotMismatchException(report);
                }
                if (changes || (_baseline.Owner is null && testPolicy == SnapshotUpdate.All))
                    _store.Commit(_baseline, desired);
                else _store.VerifyUnchanged(_baseline);
    ...
    public void Abort(Exception? error = null)
    {
        lock (_gate)
        {
            if (_state is State.Completed or State.Aborted) return;
            ...
            {
                try { error.Data["TheLithium.Imprint.Artifacts"] = ArtifactDirectory; }
    ...
    public void Dispose()
    {
        ...
        {
            if (_disposed) return;
            if (_state != State.Completed) Abort();
            _disposed = true;
