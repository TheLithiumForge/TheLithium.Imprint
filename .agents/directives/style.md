...
- Name a type for what it produces, owns, or decides, never for what it is not. A name formed by negation, such as a renderer named for being "not JSON", carries no meaning and cannot distinguish two types with the same negation. Prefer the output shape, the subject, or the layer: `DoctorFindingLine`, `ExtensionPathList`, `RouteTextRenderer`. When several types share a prefix and differ only by an opaque qualifier, the qualifier is the problem.
